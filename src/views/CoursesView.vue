<template>
  ...
  :hide-delete="!currentUserDirector" :hide-add="!currentUserDirector" :hide-edit="!currentUserDirector" icon="mdi-bullhorn-variant" :entity-property="entityProperty"  :headers="header" @edit="onEdit" @add="flagEdit=false" @detalle="onDetail">
    <template #form>
        ...
        <v-select :disabled="flagEdit" :filled="flagEdit" dense outlined v-model="entityProperty.competenceIds" hide-details :items="competencias" item-text="name" item-value="id" :menu-props="{ top: false, offsetY: true }" label="Competencias" multiple></v-select>
    </template>
